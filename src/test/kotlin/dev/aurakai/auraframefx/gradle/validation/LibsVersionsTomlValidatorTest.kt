...
class LibsVersionsTomlValidatorTest {
    ...
    @Test
    fun `validate should handle TOML with malformed inline table syntax`() {
        val malformedInlineToml = """
            [versions]
            test = "1.0.0"
            
            [libraries]
            malformed = { module = "group:artifact", version.ref = "test", extra = { unclosed = "value" }
        """.trimIndent()
    ...
    @Test
    fun `validate should handle version strings that look like regex patterns`() {
        val regexLikeToml = """
            [versions]
            regex1 = "1.*.3"
            regex2 = "2.+.0"
        ...
        testFile.writeText(regexLikeToml)
        
        val result = validator.validate()
        
        // Should detect invalid version formats that look like regex
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("Invalid version format") })
    }
    @Test
    fun `validator constructor should handle non-existent file paths gracefully`() {
        // Test with a non-existent directory path
        val nonExistentFile = File("/non/existent/path/libs.versions.toml")
        val invalidValidator = LibsVersionsTomlValidator(nonExistentFile)
        
        val result = invalidValidator.validate()
        
        assertFalse(result.isValid)
        assertEquals(listOf("TOML file does not exist"), result.errors)
    }
    @Test
    fun `validate should detect potentially dangerous version patterns`() {
        val dangerousVersionToml = """
            [versions]
            wildcard = "*"
            latest = "LATEST"
            release = "RELEASE"
        ...
        testFile.writeText(dangerousVersionToml)
        
        val result = validator.validate()
        
        // Should detect dangerous version patterns
        assertFalse(result.isValid)
        assertTrue(result.errors.any { it.contains("Invalid version format") })
    }
    @Test
    fun `ValidationResult hashCode should be consistent with equals`() {
        ...
        // Unequal objects should have different hash codes (ideally)
        assertNotEquals(result1, result3)
        assertNotEquals(result1.hashCode(), result3.hashCode())
    }
    @Test
    fun `validate should handle bundles with extremely large library lists`() {
        val largeBundleBuilder = StringBuilder()
        largeBundleBuilder.append("[versions]\ntest = \"1.0.0\"\n\n[libraries]\n")
        
        // Create 200 libraries
        for (i in 1..200) {
            largeBundleBuilder.append("bundle-lib-$i = { module = \"group:artifact$i\", version.ref = \"test\" }\n")
        }
        
        largeBundleBuilder.append("\n[bundles]\nmega-bundle = [")
        for (i in 1..200) {
            largeBundleBuilder.append("\"bundle-lib-$i\"")
            if (i < 200) largeBundleBuilder.append(", ")
        }
        largeBundleBuilder.append("]\n")
    ...
    @Test
    fun `validate should handle version reference keys with special characters`() {
        val specialRefToml = """
            [versions]
            "version-with-dashes" = "1.0.0"
            "version.with.dots" = "2.0.0"
            "version_with_underscores" = "3.0.0"
            "version with spaces" = "4.0.0"
            
            [libraries]
            lib1 = { module = "group:artifact1", version.ref = "version-with-dashes" }
            lib2 = { module = "group:artifact2", version.ref = "version.with.dots" }
            lib3 = { module = "group:artifact3", version.ref = "version_with_underscores" }
            lib4 = { module = "group:artifact4", version.ref = "version with spaces" }
        """.trimIndent()
        
        testFile.writeText(specialRefToml)
    ...
    @Test
    fun `validate should handle performance efficiently with large TOML files`() {
        val performanceTestBuilder = StringBuilder()
        performanceTestBuilder.append("[versions]\n")
        
        // Create 300 versions for performance testing
        for (i in 1..300) {
        ...
        for (i in 1..300) {
            performanceTestBuilder.append("perf-lib-$i = { module = \"group:artifact$i\", version.ref = \"perf-version-$i\" }\n")
        }
        ...
        // Should complete validation within reasonable time (less than 5 seconds)
        assertTrue(endTime - startTime < 5000, "Validation took too long: ${endTime - startTime}ms")
        assertTrue(result.isValid)
        assertTrue(result.errors.isEmpty())
    }
    @Test
    fun `ValidationResult should handle extreme values correctly`() {
        val maxResult = ValidationResult(
            isValid = true,
            errors = (1..1000).map { "Error $it" },
            warnings = (1..1000).map { "Warning $it" },
            timestamp = Long.MAX_VALUE
        )
        
        val minResult = ValidationResult(
            isValid = false,
            errors = emptyList(),
            warnings = emptyList(),
        ...
        assertEquals(1000, maxResult.errors.size)
        assertEquals(1000, maxResult.warnings.size)
        assertEquals(Long.MAX_VALUE, maxResult.timestamp)
        
        assertFalse(minResult.isValid)
        assertTrue(minResult.errors.isEmpty())
        assertTrue(minResult.warnings.isEmpty())
        assertEquals(Long.MIN_VALUE, minResult.timestamp)
    }
    @Test
    fun `validate should handle TOML with complex nested structures`() {
        val nestedToml = """
            [versions]
            kotlin = "1.9.0"
            coroutines = "1.7.3"
            ...
            [bundles]
            kotlin-stack = ["kotlin-stdlib", "kotlinx-coroutines-core"]
            ktor-stack = ["ktor-server-core", "ktor-server-netty"]
        """.trimIndent()
