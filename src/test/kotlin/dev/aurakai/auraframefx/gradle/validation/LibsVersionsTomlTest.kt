...
class LibsVersionsTomlTest {
    ...
    fun testUnreferencedVersions() {
        ...
        assertTrue(
            "TOML with unreferenced versions should pass validation but warn",
            result.isValid
        )
        assertTrue(
    ...
    fun testCriticalDependenciesPresent() {
        ...
        assertTrue(
            "TOML without critical dependencies should pass validation but warn",
            result.isValid
        )
        assertTrue(
    ...
    fun testSecurityVulnerabilityDetection() {
        ...
        assertTrue(
            "Should detect junit 4.12 as vulnerable",
            result.warnings.any { it.contains("4.12") })
        assertTrue(
            "Should detect junit 4.11 as vulnerable",
            result.warnings.any { it.contains("4.11") })
        assertTrue(
            "Should detect junit 4.10 as vulnerable",
    ...
    fun testValidationResultInternalConsistency() {
        ...
        if (result.isValid) {
            assertTrue(
                "Valid results should have no critical errors",
...
fun testTomlWithInvalidModuleNames() {
    ...
    assertTrue(
        "Should report multiple invalid module formats",
