...
class OracleDriveManagerTest {
    ...
    inner class FileUploadTests {
        ...
        fun `manageFiles should upload file successfully when security validation passes`() =
            runTest {
                // Given
                val driveFile =
                    DriveFile("file1", "test.txt", "content".toByteArray(), 1024L, "text/plain")
                ...
                val securityValidation =
                    SecurityValidation(isSecure = true, SecurityThreat("", 0, ""))
                ...
                coEvery {
                    mockCloudStorageProvider.uploadFile(
                        optimizedFile,
                        metadata
                    )
