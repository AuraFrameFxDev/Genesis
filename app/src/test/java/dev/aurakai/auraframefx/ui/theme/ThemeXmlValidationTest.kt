...
class ThemeXmlValidationTest {
    ...
    fun testXmlHasCorrectEncodingDeclaration() {
        assertTrue("XML should specify UTF-8 encoding", 
            themeXml.contains("encoding=\"utf-8\""))
        assertTrue("XML should have proper XML declaration", 
    ...
    fun testThemeHasCorrectNameAttribute() {
        ...
        assertEquals("Theme name should be 'Theme.AuraFrameFX'", 
            "Theme.AuraFrameFX", themeStyle.getAttribute("name"))
        assertFalse("Theme name should not be empty", 
    ...
    fun testThemeHasCorrectParentAttribute() {
        val styleElements = document.getElementsByTagName("style")
        val themeStyle = styleElements.item(0) as Element
        assertEquals("Parent theme should be 'Theme.AppCompat.DayNight.NoActionBar'", 
    ...
    fun testThemeContainsRequiredColorAttributes() {
        ...
        for (colorName in requiredColors) {
            assertTrue("Theme should contain $colorName", 
    ...
    fun testColorReferencesFollowAndroidResourceFormat() {
        ...
        for (i in 0 until itemElements.length) {
            ...
            if (name.contains("color") && !name.startsWith("android:")) {
                assertTrue("Color reference '$content' for '$name' should start with @color/", 
    ...
    fun testStatusBarColorIsProperlyConfigured() {
        ...
        for (i in 0 until itemElements.length) {
            ...
            if (item.getAttribute("name") == "android:statusBarColor") {
                ...
                assertEquals("Status bar should be transparent", 
    ...
    fun testXmlContainsProperCommentsForDocumentation() {
        assertTrue("Should contain comment about base application theme", 
            themeXml.contains("<!-- Base application theme. -->"))
        assertTrue("Should contain comment about primary brand color", 
            themeXml.contains("<!-- Primary brand color. -->"))
        assertTrue("Should contain comment about status bar color", 
            themeXml.contains("<!-- Status bar color. -->"))
        assertTrue("Should contain customization comment", 
    ...
    fun testColorValuesReferenceDarkThemeColors() {
        ...
        assertEquals("colorPrimary should reference dark_primary", 
            "@color/dark_primary", colorMappings["colorPrimary"])
        assertEquals("colorPrimaryDark should reference dark_primary_container", 
            "@color/dark_primary_container", colorMappings["colorPrimaryDark"])
        assertEquals("colorAccent should reference dark_secondary", 
    ...
    fun testThemeIsCompatibleWithAppCompat() {
        ...
        assertTrue("Theme should extend AppCompat theme", 
            parent.contains("Theme.AppCompat"))
        assertTrue("Theme should use DayNight variant for dark mode support", 
            parent.contains("DayNight"))
        assertTrue("Theme should have NoActionBar for custom toolbar", 
    ...
    fun testAllColorReferencesAreValidResourceFormat() {
        ...
        assertTrue("All color references should match Android format. Invalid: $invalidReferences", 
    ...
    fun testThemeNameFollowsAndroidNamingConventions() {
        ...
        assertTrue("Theme name should start with 'Theme.'", 
            themeName.startsWith("Theme."))
        assertTrue("Theme name should use PascalCase", 
            themeName.matches(Regex("^Theme\\.[A-Z][a-zA-Z0-9]*$")))
        assertFalse("Theme name should not contain spaces", 
            themeName.contains(" "))
        assertFalse("Theme name should not contain special characters", 
    ...
    fun testThemeHasAllRequiredAttributesPresent() {
        ...
        val themeStyle = styleElements.item(0) as Element
        assertTrue("Style should have name attribute", 
            themeStyle.hasAttribute("name"))
        assertTrue("Style should have parent attribute", 
            themeStyle.hasAttribute("parent"))
        assertFalse("Name attribute should not be empty", 
            themeStyle.getAttribute("name").isEmpty())
        assertFalse("Parent attribute should not be empty", 
    ...
    fun testThemeItemsHaveValidNameAttributes() {
        ...
        assertTrue("All item names should be valid. Invalid items: $invalidItems", 
    ...
    fun testThemeSupportsDarkModeConfiguration() {
        ...
        assertTrue("Theme should use DayNight for automatic dark mode switching", 
            parent.contains("DayNight"))
            ...
            when (eventType) {
                ...
                XmlPullParser.END_TAG -> {
                    if (elementStack.isNotEmpty()) {
                        ...
                        assertEquals("Closing tag should match opening tag", lastElement, parser.name)
                    }
                }
            }
            eventType = parser.next()
    ...
    fun testThemeContainsOnlyValidItemElements() {
        ...
        for (i in 0 until itemElements.length) {
            ...
            assertTrue("Item at index $i should have name attribute", 
                name.isNotEmpty())
            
            // Each item should have content
            assertTrue("Item '$name' should have content", 
                content.isNotEmpty())
            
            // Validate item names are properly formatted
            assertTrue("Item name '$name' should be valid Android attribute", 
    ...
    fun testXmlParsingPerformance() {
        ...
        assertTrue("XML parsing should complete within reasonable time (< 1000ms)", 
            duration < 1000)
    }
    ...
    fun testThemeCompatibilityWithMaterial3() {
        ...
        assertTrue("Theme should contain Material Design color attributes", 
            foundColors.isNotEmpty())
        assertTrue("Theme should have primary color for Material compatibility", 
    ...
    fun testThemeInheritanceChain() {
        ...
        assertTrue("Parent should be a valid AppCompat theme", 
            parent.startsWith("Theme.AppCompat"))
        
        // Check that it supports modern Android features
        assertTrue("Should use DayNight for system theme support", 
            parent.contains("DayNight"))
        
        // Verify NoActionBar for modern app design
        assertTrue("Should use NoActionBar for custom app bars", 
            parent.contains("NoActionBar"))
    }
}