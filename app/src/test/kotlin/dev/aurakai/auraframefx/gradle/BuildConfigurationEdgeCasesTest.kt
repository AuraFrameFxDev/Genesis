...
class BuildConfigurationEdgeCasesTest {
    ...
    fun `test build tools version is compatible`() {
        ...
        val buildToolsPattern = Regex("buildToolsVersion\s*=\s*"([^"]+)"")
        ...
        if (buildToolsVersion != null) {
            ...
            assertTrue("Build tools version should follow x.y.z format", 
    ...
    fun `test application ID format is valid`() {
        ...
        if (applicationId != null) {
            ...
            assertTrue("Application ID should follow package naming convention", 
    ...
    fun `test version name and code consistency`() {
        val versionNamePattern = Regex("versionName\s*=\s*"([^"]+)"")
        ...
        if (versionName != null) {
            ...
            assertTrue("Version name should follow semantic versioning", 
    ...
    fun `test namespace configuration is valid`() {
        ...
        if (namespace != null) {
            ...
            assertTrue("Namespace should follow package naming convention", 
    ...
    fun `test build types are properly configured`() {
        if (buildContent.contains("buildTypes")) {
            ...
            if (releaseConfigured) {
                assertTrue("Release build should have minifyEnabled configured", 
                    buildContent.contains("minifyEnabled"))
            }
            
            // Check for proper signing configuration
            if (buildContent.contains("signingConfig")) {
                assertTrue("Signing config should be properly referenced", 
        ...
        if (buildContent.contains("lint {")) {
            ...
            lintOptions.forEach { option ->
                if (buildContent.contains(option)) {
                    assertTrue("Lint option $option should have boolean value", 
    ...
    fun `test resource configuration is valid`() {
        ...
        if (buildContent.contains("shrinkResources")) {
            assertTrue("Resource shrinking should be boolean", 
        ...
        if (buildContent.contains("resConfigs")) {
            assertTrue("Resource configs should be properly quoted", 
    ...
    fun `test test instrumentation runner configuration`() {
        val testRunnerPattern = Regex("testInstrumentationRunner\s*=\s*"([^"]+)"")
        ...
        if (testRunner != null) {
            ...
            assertTrue("Test instrumentation runner should follow package naming", 
                testRunner.contains("."))
            assertFalse("Test instrumentation runner should not start with dot", testRunner.startsWith("."))
        }
    }
    ...
    fun `test external native build configuration is valid`() {
        if (buildContent.contains("externalNativeBuild")) {
            ...
            if (buildContent.contains("cmake")) {
                assertTrue("CMake block should be properly configured", 
                    buildContent.contains("cmake {"))
                
                if (buildContent.contains("path")) {
                    assertTrue("CMake path should reference CMakeLists.txt", 
    ...
    fun `test dependencies block structure is valid`() {
        if (buildContent.contains("dependencies")) {
            assertTrue("Dependencies block should be properly opened", 
            ...
            assertTrue("Should have at least one implementation dependency", 
                implementationCount > 0)
            
            // Check for proper libs reference format
            if (buildContent.contains("implementation(libs.")) {
                assertTrue("Libs references should follow proper format", 
    ...
    fun `test Java compatibility versions are consistent`() {
        ...
        val sourceCompat = sourceCompatPattern.find(buildContent)?.groupValues?.get(1)?.toIntOrNull()
        val targetCompat = targetCompatPattern.find(buildContent)?.groupValues?.get(1)?.toIntOrNull()
        ...
        if (targetCompat != null && jvmTarget != null) {
            assertEquals("Target compatibility and JVM target should match", targetCompat, jvmTarget)
        }
    }
    ...
    fun `test multiDex configuration is valid`() {
        if (buildContent.contains("multiDexEnabled")) {
            assertTrue("MultiDex should be boolean value", 
    ...
    fun `test packaging options are properly configured`() {
        if (buildContent.contains("packaging")) {
            assertTrue("Packaging block should be properly opened", 
                buildContent.contains("packaging {"))
            
            // Check for resource excludes
            if (buildContent.contains("excludes")) {
                assertTrue("Resource excludes should be properly formatted", 
    ...
    fun `test vector drawables configuration`() {
        if (buildContent.contains("vectorDrawables")) {
            assertTrue("Vector drawables block should be properly configured", 
                buildContent.contains("vectorDrawables {"))
            
            if (buildContent.contains("useSupportLibrary")) {
                assertTrue("useSupportLibrary should be boolean", 
    ...
    fun `test build file content parsing performance`() {
        ...
        assertTrue("Regex searches should complete within reasonable time", 
            (endTime - startTime) < 5000) // 5 seconds max
    }
    ...
    fun `test build file path resolution robustness`() {
        ...
        if (foundValidPath) {
            assertTrue("Found build file should contain android block", 
    ...
    fun `test KSP configuration is valid`() {
        if (buildContent.contains("ksp {")) {
            assertTrue("KSP block should be properly configured", 
            ...
            if (buildContent.contains("room.schemaLocation")) {
                ...
                if (schemaLocation != null) {
                    ...
                    assertTrue("Schema location should be a valid path", 
    ...
    fun `test OpenAPI generator configuration is valid`() {
        if (buildContent.contains("openApiGenerate")) {
            assertTrue("OpenAPI generate block should be properly configured", 
            ...
            requiredOptions.forEach { option ->
                if (buildContent.contains(option)) {
                    assertTrue("$option should be properly set", 
    ...
    fun `test Android resources configuration is valid`() {
        if (buildContent.contains("androidResources")) {
            assertTrue("Android resources block should be properly configured", 
                buildContent.contains("androidResources {"))
            
            if (buildContent.contains("noCompress")) {
                assertTrue("noCompress should be properly formatted", 
    ...
    fun `test Kotlin compiler options are valid`() {
        if (buildContent.contains("compilerOptions")) {
            assertTrue("Compiler options block should be properly configured", 
                buildContent.contains("compilerOptions {"))
            
            if (buildContent.contains("freeCompilerArgs")) {
                assertTrue("Free compiler args should be properly formatted", 
    ...
    fun `test build features configuration is valid`() {
        if (buildContent.contains("buildFeatures")) {
            assertTrue("Build features block should be properly configured", 
            ...
            buildFeatures.forEach { feature ->
                if (buildContent.contains(feature)) {
                    assertTrue("$feature should be boolean value", 
    ...
    fun `test source sets configuration is valid`() {
        if (buildContent.contains("sourceSets")) {
            assertTrue("Source sets should be properly configured", 
                buildContent.contains("sourceSets") || buildContent.contains("android.sourceSets"))
            
            if (buildContent.contains("srcDirs")) {
                assertTrue("Source directories should be properly formatted", 
    ...
    fun `test configurations block is valid`() {
        if (buildContent.contains("configurations.all")) {
            assertTrue("Configurations block should be properly structured", 
                buildContent.contains("configurations.all {"))
            
            if (buildContent.contains("exclude")) {
                assertTrue("Exclude statements should be properly formatted", 
