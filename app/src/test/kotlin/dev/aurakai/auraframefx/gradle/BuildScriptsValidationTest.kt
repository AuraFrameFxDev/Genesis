...
class BuildScriptsValidationTest {
    ...
    fun `test version configuration is present`() {
        ...
        assertTrue("versionName should be set", buildGradleContent.contains("versionName = \"1.0\""))
    }
    ...
    fun `test required build features are enabled`() {
        assertTrue("buildConfig should be enabled", buildGradleContent.contains("buildConfig = true"))
        assertTrue("compose should be enabled", buildGradleContent.contains("compose = true"))
        assertTrue("viewBinding should be enabled", buildGradleContent.contains("viewBinding = true"))
    ...
    fun `test packaging configuration is present`() {
        ...
        assertTrue("Resources exclusions should be configured", buildGradleContent.contains("resources {"))
    ...
    fun `test cmake configuration is present`() {
        assertTrue("CMake external build should be configured", buildGradleContent.contains("externalNativeBuild {"))
        assertTrue("CMake path should be set", buildGradleContent.contains("path = file(\"src/main/cpp/CMakeLists.txt\")"))
        assertTrue("CMake version should reference root project", 
    ...
    fun `test lint configuration is comprehensive`() {
        ...
        assertTrue("Lint baseline should be configured", buildGradleContent.contains("baseline = file(\"lint-baseline.xml\")"))
        assertTrue("Check dependencies should be enabled", buildGradleContent.contains("checkDependencies = true"))
        assertTrue("Lint config should be set", buildGradleContent.contains("lintConfig = file(\"lint.xml\")"))
        assertTrue("Warnings as errors should be enabled", buildGradleContent.contains("warningsAsErrors = true"))
        assertTrue("Abort on error should be enabled", buildGradleContent.contains("abortOnError = true"))
    }
    ...
    fun `test openapi generator configuration`() {
        assertTrue("OpenAPI generator block should exist", buildGradleContent.contains("openApiGenerate {"))
        assertTrue("Generator name should be kotlin", buildGradleContent.contains("generatorName.set(\"kotlin\")"))
        assertTrue("Input spec should be configured", buildGradleContent.contains("inputSpec.set("))
        assertTrue("Output directory should be configured", buildGradleContent.contains("outputDir.set("))
        assertTrue("API package should be set", buildGradleContent.contains("apiPackage.set(\"dev.aurakai.auraframefx.api.client.apis\")"))
        assertTrue("Model package should be set", buildGradleContent.contains("modelPackage.set(\"dev.aurakai.auraframefx.api.client.models\")"))
    }

    @Test
    fun `test openapi generator options`() {
        assertTrue("Date library should be java8", buildGradleContent.contains("\"dateLibrary\" to \"java8\""))
        assertTrue("Coroutines should be enabled", buildGradleContent.contains("\"useCoroutines\" to \"true\""))
        assertTrue("Kotlinx serialization should be used", 
    ...
    fun `test source sets configuration`() {
        assertTrue("Main source set should be configured", 
            buildGradleContent.contains("android.sourceSets.getByName(\"main\")"))
        assertTrue("Generated OpenAPI sources should be included", 
    ...
    fun `test preBuild task dependencies`() {
        assertTrue("preBuild task should be configured", buildGradleContent.contains("tasks.named(\"preBuild\")"))
        assertTrue("preBuild should depend on openApiGenerate", 
    ...
    fun `test core dependencies are present`() {
        ...
        coreDependencies.forEach { dependency ->
            assertTrue("Core dependency $dependency should be present", 
    ...
    fun `test compose bom is used`() {
        assertTrue("Compose BOM should be used", 
            buildGradleContent.contains("val composeBom = platform(libs.compose.bom)"))
        assertTrue("Compose BOM should be implemented", 
            buildGradleContent.contains("implementation(composeBom)"))
        assertTrue("Compose BOM should be used in android tests", 
    ...
    fun `test compose dependencies are comprehensive`() {
        ...
        composeDeps.forEach { dependency ->
            assertTrue("Compose dependency $dependency should be present", 
                buildGradleContent.contains(dependency))
        }
    }
    ...
    fun `test hilt dependencies are complete`() {
        ...
        hiltDeps.forEach { dependency ->
            assertTrue("Hilt dependency $dependency should be present", 
                buildGradleContent.contains(dependency))
        }
    }
    ...
    fun `test room dependencies use bundles`() {
        assertTrue("Room bundle should be used", buildGradleContent.contains("implementation(libs.bundles.room)"))
        assertTrue("Room compiler should be configured with KSP", buildGradleContent.contains("ksp(libs.room.compiler)"))
    }

    @Test
    fun `test firebase dependencies use bom and bundles`() {
        assertTrue("Firebase BOM should be used", buildGradleContent.contains("implementation(platform(libs.firebase.bom))"))
        assertTrue("Firebase bundle should be used", buildGradleContent.contains("implementation(libs.bundles.firebase)"))
    }
    ...
    fun `test network dependencies are present`() {
        ...
        networkDeps.forEach { dependency ->
            assertTrue("Network dependency $dependency should be present", 
                buildGradleContent.contains(dependency))
        }
    }
    ...
    fun `test testing dependencies use bundles`() {
        assertTrue("Unit testing bundle should be used", 
            buildGradleContent.contains("testImplementation(libs.bundles.testing.unit)"))
        assertTrue("Android testing bundle should be used", 
    ...
    fun `test lifecycle dependencies use bundles`() {
        assertTrue("Lifecycle bundle should be used", 
    ...
    fun `test deprecated configurations are removed`() {
        ...
        deprecatedPatterns.forEach { pattern ->
            assertFalse("Deprecated pattern $pattern should not be present", 
    ...
    fun `test comments indicate moved configurations`() {
        assertTrue("Should have comment about moved NDK ABI filters", 
            buildGradleContent.contains("// MOVED: NDK ABI filters"))
        assertTrue("Should have comment about moved packaging options", 
            buildGradleContent.contains("// MOVED: Packaging options"))
        assertTrue("Should have comment about removed composeOptions", 
    ...
    fun `test build script handles missing files gracefully`() {
        ...
        assertFalse("Should not reference non-existent proguard files directly", 
    ...
    fun `test version catalog references are consistent`() {
        ...
        libsReferences.forEach { ref ->
            assertTrue("Libs reference $ref should follow naming convention", 
    ...
    fun `test no hardcoded paths in configuration`() {
        ...
        hardcodedPathPatterns.forEach { pattern ->
            assertFalse("Should not contain hardcoded path $pattern", 
                buildGradleContent.contains(pattern))
        }
    }
    ...
    fun `test optimization settings are appropriate`() {
        assertTrue("Release minification should be enabled", 
            buildGradleContent.contains("isMinifyEnabled = true"))
        assertTrue("Should use optimized ProGuard file", 
    ...
    fun `test debug-only dependencies are properly scoped`() {
        ...
        assertTrue("Compose UI tooling should be debug-only", 
            buildGradleContent.contains("debugImplementation(libs.compose.ui.tooling)"))
        assertTrue("Compose test manifest should be debug-only", 
    ...
    fun `test security dependencies are included`() {
        ...
        assertTrue("DataStore preferences should be included for secure storage", 
    ...
    fun `test build script structure is valid kotlin dsl`() {
        ...
        assertTrue("Should use proper block structure", buildGradleContent.contains("dependencies {"))
    ...
    fun `test all required sections are present in correct order`() {
        ...
        expectedSections.forEach { section ->
            ...
            assertTrue("Section $section should come after previous sections", currentIndex > lastIndex)
