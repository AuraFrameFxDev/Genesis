...
class VersionCatalogTest {
    ...
    fun `test version catalog is used consistently`() {
        ...
        val unexpectedVersions = hardcodedVersions.map { it.value.trim('"') }
            .filterNot { version -> allowedHardcodedVersions.any { allowed -> version.contains(allowed) } }
        
        assertTrue("Should minimize hardcoded versions in favor of version catalog: $unexpectedVersions", 
    ...
    fun `test no direct dependency declarations`() {
        ...
        directDependencyPatterns.forEach { pattern ->
            ...
            assertTrue("Should minimize direct dependency declarations in favor of version catalog", 
                invalidMatches <= 2) // Allow some minimal exceptions
        }
    }
    ...
    fun `test build file exists and is readable`() {
        ...
        assertTrue("Build file should contain basic Gradle structures", 
    ...
    fun `test version catalog references follow naming conventions`() {
        ...
        libsReferences.forEach { match ->
            ...
            assertFalse("Library reference should not contain underscores: $reference", 
                nameAfterLibs.contains("_"))
            assertFalse("Library reference should not start with number: $reference", 
    ...
    fun `test plugin alias format consistency`() {
        ...
        pluginAliases.forEach { match ->
            ...
            assertTrue("Plugin alias should be properly formatted: $alias", 
    ...
    fun `test no deprecated dependency syntax`() {
        ...
        assertFalse("Should not use deprecated 'compile' configuration", 
            buildContent.contains("compile "))
        assertFalse("Should not use deprecated 'testCompile' configuration", 
            buildContent.contains("testCompile "))
        assertFalse("Should not use deprecated 'androidTestCompile' configuration", 
    ...
    fun `test consistent spacing in dependency declarations`() {
        ...
        dependencyPatterns.forEach { pattern ->
            ...
            matches.forEach { match ->
                ...
                assertTrue("Dependency declaration should have proper spacing: ${match.value}", 
    ...
    fun `test version catalog bundles usage`() {
        ...
        bundleReferences.forEach { match ->
            ...
            assertFalse("Bundle name should not contain underscores: $bundle", 
    ...
    fun `test BOM platform dependencies are properly handled`() {
        ...
        expectedBoms.forEach { bom ->
            if (buildContent.contains("libs.$bom")) {
                assertTrue("BOM $bom should be used with platform(): expected platform(libs.$bom)", 
    ...
    fun `test version reference syntax consistency`() {
        val versionReferences = Regex("libs\\.versions\\.[a-zA-Z0-9\\.]+\\.get\\(\\)").findAll(buildContent)
        
        versionReferences.forEach { match ->
            val reference = match.value
            assertTrue("Version reference should end with .get(): $reference", 
                reference.endsWith(".get()"))
            
            val versionName = reference.substring(13, reference.length - 6) // Remove "libs.versions." and ".get()"
            assertFalse("Version name should not contain underscores: $reference", 
    ...
    fun `test implementation vs api usage patterns`() {
        ...
        if (apiCount > 0) {
            assertTrue("Should use 'implementation' more frequently than 'api' for better encapsulation", 
    ...
    fun `test no hardcoded build tool versions should use catalog when possible`() {
        ...
        assertTrue("Build tool versions found - consider version catalog for consistency", 
            foundHardcodedVersions >= 0) // Always pass but log findings
    }
    ...
    fun `test proper multiline dependency formatting`() {
        ...
        dependencyBlocks.forEach { block ->
            ...
            lines.forEach { line ->
                if (line.contains("implementation") || line.contains("testImplementation") || line.contains("api")) {
                    assertTrue("Dependency lines should be properly indented: '$line'", 
    ...
    fun `test version catalog availability edge cases`() {
        ...
        assertEquals("Empty build file should have no hardcoded versions", 0, hardcodedVersions.count())
    }
    ...
    fun `test specific plugin version references`() {
        ...
        if (buildContent.contains("org.jetbrains.kotlin.plugin.compose")) {
            assertTrue("Kotlin compose plugin should use version catalog", 
    ...
    fun `test no duplicate plugin applications`() {
        ...
        val pluginPattern = Regex("(alias\\(libs\\.plugins\\.[a-zA-Z0-9\\.]+\\)|id\\(\"[^\"]+\"\\))")
        ...
        pluginApplications.forEach { (plugin, count) ->
            assertTrue("Plugin should not be applied multiple times: $plugin (found $count times)", 
                count <= 1)
        }
    }
    ...
    fun `test proper dependency scoping`() {
        ...
        val testDependencies = Regex("testImplementation\\(libs\\.[a-zA-Z0-9\\.]+\\)").findAll(buildContent)
        val androidTestDependencies = Regex("androidTestImplementation\\(libs\\.[a-zA-Z0-9\\.]+\\)").findAll(buildContent)
        ...
        if (buildContent.contains("libs.testJunit")) {
            assertTrue("JUnit should be in testImplementation scope", 
    ...
    fun `test namespace consistency`() {
        ...
        if (namespaceMatch != null) {
            ...
            assertTrue("Namespace should follow reverse domain notation", 
    ...
    fun `test version catalog usage completeness`() {
        ...
        val allDependencies = Regex("(implementation|testImplementation|androidTestImplementation|api|debugImplementation)\\s*\\(")
            .findAll(buildContent).count()
        val catalogDependencies = Regex("(implementation|testImplementation|androidTestImplementation|api|debugImplementation)\\s*\\(\\s*libs\\.")
            .findAll(buildContent).count()
        
        if (allDependencies > 0) {
            val catalogUsagePercentage = (catalogDependencies.toDouble() / allDependencies.toDouble()) * 100
            assertTrue("Should use version catalog for majority of dependencies (${catalogUsagePercentage.toInt()}%)", 
